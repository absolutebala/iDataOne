...
.nav-hamburger{display:none;position:fixed;top:14px;right:16px;z-index:201;width:40px;height:40px;border-radius:10px;background:rgba(255,255,255,0.92);backdrop-filter:blur(12px);border:1px solid rgba(99,102,241,0.15);flex-direction:column;align-items:center;justify-content:center;gap:5px;cursor:pointer;box-shadow:0 2px 12px rgba(15,23,42,0.1)}
.nav-hamburger span{display:block;width:18px;height:2px;background:#475569;border-radius:2px;transition:all 0.3s}
.nav-hamburger.open span:nth-child(1){transform:translateY(7px) rotate(45deg)}
.nav-hamburger.open span:nth-child(2){opacity:0}
.nav-hamburger.open span:nth-child(3){transform:translateY(-7px) rotate(-45deg)}
.nav-mob-menu{position:fixed;top:0;right:-100%;width:280px;height:100vh;background:#fff;z-index:200;box-shadow:-4px 0 40px rgba(15,23,42,0.15);transition:right 0.35s cubic-bezier(0.4,0,0.2,1);padding:80px 32px 40px;display:flex;flex-direction:column;gap:4px}
.nav-mob-menu.open{right:0}
.nav-mob-menu a{font-size:16px;font-weight:500;color:#475569;text-decoration:none;padding:14px 0;border-bottom:1px solid rgba(226,232,240,0.6);display:block;transition:color 0.2s}
.nav-mob-menu a:hover{color:#4f46e5}
.nav-mob-menu a.active{color:#4f46e5;font-weight:600}
.nav-mob-overlay{position:fixed;inset:0;background:rgba(15,23,42,0.3);z-index:199;opacity:0;pointer-events:none;transition:opacity 0.3s}
.nav-mob-overlay.open{opacity:1;pointer-events:auto}
@media(max-width:768px){.site-nav .nav-links{display:none}.nav-hamburger{display:flex}}
</style>

<div class="nav-hamburger" id="navHamburger" onclick="toggleNavMenu()">
  <span></span><span></span><span></span>
</div>
<div class="nav-mob-overlay" id="navOverlay" onclick="toggleNavMenu()"></div>
...
function toggleNavMenu(){
  ...
  document.getElementById('navMobMenu').classList.toggle('open');
