...
<title>Case Study: Risk Intelligence Dashboard — iDataOne</title>
<meta name="description" content="How we built a real-time risk intelligence dashboard with severity matrix visualisation, trend tracking and automated compliance scoring for an enterprise FMCG company.">
...
.cs-hero{background:linear-gradient(135deg,#0f172a 0%,#1c1917 50%,#0f172a 100%);padding:80px 32px 60px;position:relative;overflow:hidden}
...
.cs-badge{display:inline-block;padding:5px 14px;border-radius:999px;background:rgba(217,119,6,0.15);border:1px solid rgba(217,119,6,0.25);font-size:11px;font-weight:600;letter-spacing:2px;text-transform:uppercase;color:#fbbf24;margin-bottom:24px}
.cs-hero-title{font-size:clamp(32px,5vw,48px);font-weight:800;letter-spacing:-2px;line-height:1.08;color:#fff;margin-bottom:20px}
.cs-hero-sub{font-size:17px;color:rgba(255,255,255,0.45);line-height:1.75;margin-bottom:40px;max-width:600px}
.cs-hero-stats{display:flex;gap:48px;padding-top:32px;border-top:1px solid rgba(255,255,255,0.08)}
.cs-stat{text-align:left}
.cs-stat-num{font-size:32px;font-weight:800;letter-spacing:-1.5px;color:#fff}
...
@media(max-width:768px){.cs-hero-stats{flex-wrap:wrap;gap:24px}.cs-grid{grid-template-columns:1fr}.cs-features{grid-template-columns:1fr}.cs-results{grid-template-columns:1fr 1fr}.cs-cta-card{flex-direction:column;padding:32px 24px;text-align:center}}
</style>
</head>
<body>
